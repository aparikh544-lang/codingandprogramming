...
import { Star, Heart, MapPin, Phone, Tag, ExternalLink, Crown } from 'lucide-react';
  ...
  return (
    <Link to={`/business/${business.id}`}>
      <div className={`bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow ${
        business.featured ? 'ring-2 ring-yellow-400 relative' : ''
      }`}>
        {business.featured && (
          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-yellow-400 via-amber-500 to-yellow-400 z-10"></div>
        )}
        <div className="relative h-48">
          ...
          {business.featured && (
            <div className="absolute top-3 left-3 bg-gradient-to-r from-yellow-400 to-amber-500 text-white px-3 py-1 rounded-full text-sm font-bold flex items-center gap-1 shadow-lg">
              <Crown className="w-4 h-4" />
              Featured
            </div>
          )}
          {business.hasDeal && !business.featured && (
          ...
          {business.hasDeal && business.featured && (
            <div className="absolute top-12 left-3 bg-orange-500 text-white px-3 py-1 rounded-full text-sm font-medium flex items-center gap-1">
              <Tag className="w-4 h-4" />
              Deal
            </div>
          )}
