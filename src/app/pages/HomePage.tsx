...
export default function HomePage() {
  ...
  // ✅ Reload businesses when page becomes visible (e.g., after creating a business)
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        console.log('🔄 Page visible - refreshing businesses...');
  ...
  const loadMockBusinesses = async () => {
    ...
    // ✅ Also fetch user-created businesses from Supabase
    try {
      ...
      if (!error && data && data.length > 0) {
        console.log('📦 Found user-created businesses:', data.length);
        
        // Transform user businesses to Business format
        const userBusinesses: Business[] = data.map((biz: any) => ({
          id: `user-${biz.id}`,
          name: biz.name,
        ...
        // Combine user businesses with mock businesses
        const combinedBusinesses = [...userBusinesses, ...storedBusinesses];
        setBusinesses(combinedBusinesses);
        storage.setBusinesses(combinedBusinesses);
  ...
  const loadNYCDemo = async () => {
    ...
    if (!user) {
      alert('Please log in first to use real location data!');
      return;
    }
    ...
    try {
      ...
      if (nearbyBusinesses.length === 0) {
        alert('No businesses found in this area. Try a different location!');
        loadMockBusinesses();
        return;
      }
  ...
  const filteredAndSortedBusinesses = businesses
    ...
    .sort((a, b) => {
      // ⭐ PRIORITY 1: Featured businesses always appear first
      if (a.featured && !b.featured) return -1;
      if (!a.featured && b.featured) return 1;
      
      // PRIORITY 2: Then sort by user's selected criteria
      switch (sortBy) {
        case 'rating-desc':
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-blue-800 font-medium">Auto-location not available in preview mode</p>
                  <p className="text-blue-700 text-sm">No worries! Click below to use a demo location or enter your coordinates.</p>
                </div>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={loadNYCDemo}
                  className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors whitespace-nowrap font-medium"
                >
                  Try NYC Demo
                </button>
                <button
                  onClick={() => setShowManualEntry(!showManualEntry)}
                  className="bg-white text-blue-600 border border-blue-600 px-4 py-2 rounded-lg hover:bg-blue-50 transition-colors whitespace-nowrap"
                >
                  Enter Location
                </button>
              </div>
